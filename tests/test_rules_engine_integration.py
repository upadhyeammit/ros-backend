...
class TestRulesEngineIntegration:
    ...
    def test_complete_workflow_idle_system(self, mock_pricing, mock_instances):
        ...
        mock_instances.get.return_value = {
            "ram": 1,
            "extra": {"vcpu": "1", "physicalProcessor": "Intel Xeon Family"}
        }
        ...
        mock_instances.keys.return_value = ["t2.nano", "t2.micro", "t2.small"]

        pricing_instance = Mock()
        pricing_instance.get.return_value = {"us-east-1": 0.0116}
        mock_pricing.return_value = pricing_instance

        # Create test data for idle system
        rhel = Mock()
        rhel.rhel = "8.4"
        ...
        cpu_candidates = rules_engine.cpu_candidates(cloud, cpu_eval, psi_eval)
        mem_candidates = rules_engine.mem_candidates(cloud, mem_eval, psi_eval)
        io_candidates = rules_engine.io_candidates(cloud, io_eval)

        # Test find_solution
        solution = rules_engine.find_solution(
            cloud, cpu_candidates, mem_candidates, io_candidates,
            idle_eval, psi_eval
        )
        ...
        report_result = rules_engine.report(rhel, cloud, solution)
        assert report_result is not None

    @patch('ros.rules.rules_engine.EC2_INSTANCE_TYPES')
    @patch('ros.rules.rules_engine.Ec2LinuxPrices')
    def test_complete_workflow_undersized_system(self, mock_pricing,
                                                 mock_instances):
        ...
        # Create test data for undersized system
        rhel = Mock()
        rhel.rhel = "8.4"
        ...
        io_candidates = rules_engine.io_candidates(
            cloud, rules_engine.io_evaluation(io_ut)
        )
    ...
    @patch('ros.rules.rules_engine.EC2_INSTANCE_TYPES')
    @patch('ros.rules.rules_engine.Ec2LinuxPrices')
    def test_complete_workflow_oversized_system(self, mock_pricing,
                                                mock_instances):
        """Test complete workflow for an oversized system."""
        # Setup mocks for larger instance
        mock_instances.get.return_value = {
            "ram": 16,
            "extra": {"vcpu": "4", "physicalProcessor": "Intel Xeon Family"}
        }
        mock_instances.items.return_value = [
            ("t2.nano", {
                "ram": 0.5,
        ...
        mock_instances.keys.return_value = [
            "t2.nano", "t2.micro", "t2.small", "m5.xlarge"
        ]

        pricing_instance = Mock()
        # Higher price for larger instance
        pricing_instance.get.return_value = {"us-east-1": 0.192}
        mock_pricing.return_value = pricing_instance

        # Create test data for oversized system
        rhel = Mock()
        rhel.rhel = "8.4"

        cloud = Mock()
        cloud.provider = "aws"
        cloud.type = "m5.xlarge"
        cloud.region = "us-east-1"
...
class TestRulesEngineErrorHandling:
    ...
    def test_cloud_metadata_unsupported_provider(self):
        ...
        cp = Mock()
        cp.cloud_provider = "gcp"  # Unsupported
        cp.AWS = "aws"
