...
class TestCloudMetadata:
    ...
    def test_cloud_metadata_aws_success(self):
        ...
        # Mock AWS instance data
        aws = {
            'instanceType': 't2.micro',
            'region': 'us-east-1'
        }

        # Mock Azure (None for AWS case)
        azure = None

        result = rules_engine.cloud_metadata(cp, aws, azure)
    ...
    def test_cloud_metadata_aws_with_azure_fallback(self):
        """Test AWS metadata with Azure fallback when AWS data is None."""
        cp = Mock()
        cp.cloud_provider = "aws"
        cp.AWS = "aws"
        ...
        aws = None
        azure = Mock()
        azure.raw = "Standard_D2s_v3"

        result = rules_engine.cloud_metadata(cp, aws, azure)

        assert result.provider == "aws"
        assert result.type == "Standard_D2s_v3"
        assert result.mic is None
        assert result.region is None

    def test_cloud_metadata_unsupported_provider(self):
        """Test that unsupported cloud providers raise SkipComponent."""
        cp = Mock()
...
class TestNopmlogSummary:
    ...
    def test_no_pmlog_summary_with_ros_collect_enabled(self):
        """Test when ros_collect is enabled but no pmlog summary."""
        cli_cfg = Mock()
        cli_cfg.has_option.return_value = True
        cli_cfg.getboolean.return_value = True

        new_pm = None

        result = rules_engine.no_pmlog_summary(cli_cfg, new_pm)
        assert result == "NO_PCP_DATA"

    def test_no_pmlog_summary_with_data_available(self):
        ...
        with pytest.raises(SkipComponent):
            rules_engine.no_pmlog_summary(cli_cfg, new_pm)

    def test_no_pmlog_summary_ros_collect_disabled(self):
        """Test when ros_collect is disabled - should skip."""
        cli_cfg = Mock()
        cli_cfg.has_option.return_value = False
...
class TestCpuUtilization:
    ...
    def test_cpu_utilization_with_lscpu_fallback(self):
...
class TestPsiEnabled:
    ...
    def test_psi_enabled_true(self):
        """Test PSI enabled when psi=1 in cmdline."""
        cmdline = Mock()
        cmdline.__contains__ = Mock(return_value=True)
        cmdline.__getitem__ = Mock(return_value=['1'])

        result = rules_engine.psi_enabled(cmdline)
        assert result is True

    def test_psi_enabled_false(self):
        """Test PSI disabled when psi=0 in cmdline."""
        cmdline = Mock()
        cmdline.__contains__ = Mock(return_value=True)
        cmdline.__getitem__ = Mock(return_value=['0'])
...
class TestCandidateFunctions:
    ...
    def test_cpu_candidates_undersized(self, mock_instances):
        """Test CPU candidate generation for undersized instances."""
        # Mock instance types
        mock_instances.get.return_value = {
            "extra": {"vcpu": "1", "physicalProcessor": "Intel"}
        }
        mock_instances.items.return_value = [
            ...
            ("t2.medium", {
                "extra": {"vcpu": "2", "physicalProcessor": "Intel"}
            }),
            ("m5.large", {
                "extra": {"vcpu": "2", "physicalProcessor": "AMD"}
            })  # Different processor
        ]
    ...
    @patch('ros.rules.rules_engine.EC2_INSTANCE_TYPES')
    def test_mem_candidates_undersized(self, mock_instances):
        """Test memory candidate generation for undersized instances."""
        # Mock instance types
        mock_instances.get.return_value = {"ram": 1}  # 1 GB
        mock_instances.items.return_value = [
            ("t2.small", {"ram": 2}),  # 2 GB
            ("t2.medium", {"ram": 4}),  # 4 GB
            ("t2.nano", {"ram": 0.5})  # 0.5 GB
        ]
        ...
        result_ut, candidates = rules_engine.mem_candidates(cm, mem_ut, psi_ut)

        assert result_ut & RosKeys.MEMORY_UNDERSIZED
        assert isinstance(candidates, set)

    @patch('ros.rules.rules_engine.EC2_INSTANCE_TYPES')
    def test_io_candidates(self, mock_instances):
        """Test I/O candidate generation."""
        # Mock instance types
        mock_instances.keys.return_value = [
        ...
        result_ut, candidates = rules_engine.io_candidates(cm, io_ut)

        assert result_ut == RosKeys.OPTIMIZED
        assert isinstance(candidates, set)
        assert len(candidates) == 3  # All instance types returned
...
class TestFindSolution:
    ...
    def test_find_solution_idle(self, mock_pricing):
        """Test find solution for idle system."""
        # Mock pricing
        mock_pricing.return_value.get.return_value = {"us-east-1": 0.0116}
        ...
        result = rules_engine.find_solution(cm, cpu, mem, io, idle, psi)

        err_key, cur_price, states, solution_w_price = result
        assert err_key == "INSTANCE_IDLE"
        assert cur_price == 0.0116
        assert isinstance(states, dict)
        assert isinstance(solution_w_price, list)

    @patch('ros.rules.rules_engine.Ec2LinuxPrices')
    def test_find_solution_undersized(self, mock_pricing):
...
class TestRuleFunctions:
    ...
    def test_report_metadata(self):
        ...
        result = rules_engine.report_metadata(rhel, cloud, psi, cpu, mem, io)
    ...
    def test_report_rule(self):
        ...
        result = rules_engine.report(rhel, cloud, solution)
